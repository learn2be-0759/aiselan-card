...
(function(){
if(window.parent.document.getElementById('aiselan-v3-styles'))return;
var s=window.parent.document.createElement('style');
s.id='aiselan-v3-styles';
s.textContent=CSS;
...
if(!text)return;
// 查找 [状态]...[/状态] 块
var m=text.match(/\[状态\]([\s\S]*?)\[\/状态\]/);
if(!m)return;
var block=m[1].trim();
console.log('[艾瑟兰面板] 解析状态数据...');
var lines=block.split(/\n+/);
var st=JSON.parse(JSON.stringify(state)); // deep clone
...
if(cat==='敌对'){
st.scene.hostile=parseUnits(parts.slice(1));
}else if(cat==='中立'){
st.scene.neutral=parseUnits(parts.slice(1));
}else if(cat==='友方'){
st.scene.friendly=parseUnits(parts.slice(1));
}else if(cat==='调查'){
st.scene.investigable=parseInvestigable(parts.slice(1));
}else if(cat==='时间'){
if(parts[1])st.status.situation.time=parts[1];
if(parts[2])st.status.situation.location=parts[2];
if(parts[3])st.status.situation.place=parts[3];
}else if(cat==='主角'){
parsePersonal(parts.slice(1),st.status.personal);
}else if(cat==='等级'){
st.status.personal.level=parseInt(parts[1])||1;
}else if(cat==='属性'){
var attrs=st.status.personal.attributes;
parts.slice(1).forEach(function(p){
var kv=p.split(':');if(kv.length===2){
if(kv[0]==='力')attrs.str=parseInt(kv[1])||0;
if(kv[0]==='敏')attrs.agi=parseInt(kv[1])||0;
if(kv[0]==='体')attrs.con=parseInt(kv[1])||0;
if(kv[0]==='精')attrs.spr=parseInt(kv[1])||0;
}
});
}else if(cat==='装备'){
st.status.personal.equipment=parts.slice(1).filter(Boolean);
}else if(cat==='天赋'){
st.status.personal.talents=parts.slice(1).filter(Boolean);
}else if(cat==='小队'){
parsePartyMember(parts.slice(1),st.status.party);
}else if(cat==='货币'){
parseCurrency(parts.slice(1),st.inventory.currency);
}else if(cat==='物品'){
st.inventory.items=parseItems(parts.slice(1));
}
});
...
function parseUnits(parts){
return parts.filter(Boolean).map(function(p){
// 格式: 姓名(HP/MaxHP,状态1,状态2)
var m2=p.match(/^(.+?)\((\d+)\/(\d+)(?:,(.+?))?\)$/);
if(m2){
var statuses=(m2[4]||'').split(',').filter(Boolean).map(function(s){return{label:s.trim(),type:'stamina',active:true};});
return{id:'u'+Date.now()+Math.random(),name:m2[1].trim(),hp:parseInt(m2[2]),maxHp:parseInt(m2[3]),status:statuses};
}
return{id:'u'+Date.now()+Math.random(),name:p.trim(),hp:100,maxHp:100,status:[]};
});
}

function parseInvestigable(parts){
return parts.filter(Boolean).map(function(p){return{id:'i'+Date.now()+Math.random(),name:p.trim(),type:'info'};});
}

function parsePersonal(parts,personal){
parts.forEach(function(p){
var kv=p.split(':');if(kv.length===2){
var k=kv[0],v=kv[1];
var m2=v.match(/^(\d+)\/(\d+)$/);
if(m2){
var cur=parseInt(m2[1]),max=parseInt(m2[2]);
if(k==='HP'){personal.hp=cur;personal.maxHp=max;}
else if(k==='MP'){personal.mp=cur;personal.maxMp=max;}
else if(k==='体力'){personal.stamina=cur;personal.maxStamina=max;}
else if(k==='能量'){personal.energy=cur;personal.maxEnergy=max;}
else if(k==='水分'){personal.hydration=cur;personal.maxHydration=max;}
else if(k==='疲劳'){personal.fatigue=cur;personal.maxFatigue=max;}
}
}
});
}

function parsePartyMember(parts,party){
// 格式: 姓名:HPxx/xx,体xx/xx,能xx/xx,水xx/xx,好感xx
var name=parts[0];
if(!name)return;
var member={id:'p'+Date.now()+Math.random(),name:name,hp:100,maxHp:100,stamina:80,maxStamina:80,energy:70,maxEnergy:100,hydration:80,maxHydration:100,affection:0};
parts.slice(1).forEach(function(p){
var kv=p.split(':');if(kv.length===2){
var k=kv[0],v=kv[1];
var m2=v.match(/^(\d+)\/(\d+)$/);
if(m2){
if(k==='HP'){member.hp=parseInt(m2[1]);member.maxHp=parseInt(m2[2]);}
else if(k==='体'){member.stamina=parseInt(m2[1]);member.maxStamina=parseInt(m2[2]);}
else if(k==='能'){member.energy=parseInt(m2[1]);member.maxEnergy=parseInt(m2[2]);}
else if(k==='水'){member.hydration=parseInt(m2[1]);member.maxHydration=parseInt(m2[2]);}
}
if(k==='好感'){member.affection=parseInt(v)||0;}
}
});
party.push(member);
}

function parseCurrency(parts,cur){
parts.forEach(function(p){
var kv=p.split(':');if(kv.length===2){
if(kv[0]==='金')cur.gold=parseInt(kv[1])||0;
if(kv[0]==='银')cur.silver=parseInt(kv[1])||0;
if(kv[0]==='铜')cur.copper=parseInt(kv[1])||0;
}
});
}
...
return parts.filter(Boolean).map(function(p){
var m2=p.match(/^(.+?)×(\d+)\((.+?),(.+?)\)$/); // 名称×数量(类型,稀有度)
if(m2)return{id:'it'+Date.now()+Math.random(),name:m2[1].trim(),quantity:parseInt(m2[2]),typeLabel:m2[3].trim(),type:m2[3].trim(),rarity:m2[4].trim()};
var m3=p.match(/^(.+?)\((.+?),(.+?)\)$/); // 名称(类型,稀有度)
if(m3)return{id:'it'+Date.now()+Math.random(),name:m3[1].trim(),quantity:1,typeLabel:m3[2].trim(),type:m3[2].trim(),rarity:m3[3].trim()};
return{id:'it'+Date.now()+Math.random(),name:p.trim(),quantity:1,typeLabel:'杂项',type:'misc',rarity:'common'};
});
}
...
var cards=us.length?us.map(function(u){var hpP=pct(u.hp,u.maxHp),bc=hpCls(hpP),tags=(u.status||[]).map(function(s){var cls=s.type||'';return'<span class="ais-unit-tag '+cls+'">'+esc(s.label||s)+'</span>';}).join('');return'<div class="ais-unit" title="'+esc(u.name||'')+'"><div class="ais-unit-name">'+esc(u.name||'?')+'</div><div class="ais-unit-hp"><span class="ais-unit-hp-label">生命</span><div class="ais-unit-hp-wrap"><div class="ais-unit-hp-bar '+bc+'" style="width:'+hpP+'%"></div></div><span class="ais-unit-hp-val">'+(u.hp||0)+'/'+(u.maxHp||0)+'</span></div>'+(tags?'<div class="ais-unit-tags">'+tags+'</div>':'')+'</div>';}).join(''):'<div class="ais-empty"><span>暂无</span></div>';
...
var invHTML=inv.length?inv.map(function(it){var dc=it.type||'info';return'<div class="ais-inv-item"><span class="ais-inv-dot '+dc+'"></span>'+I.search+'<span>'+esc(it.name||it)+'</span></div>';}).join(''):'<span class="ais-muted">暂无</span>';
